...
namespace Lockstep.Game
{
    public class ClientMessageDispatcher
    {
        ...
        private readonly KcpClient m_network;

        public ClientMessageDispatcher(KcpClient network)
        {
            m_network = network;
        }
        ...
        public bool Response(IMessage packet, ushort messageId, int rpcId)
        {
            if (packet == null)
            {
                return false;
            ...
            return Send(packet, messageId, rpcId);
        }

        public bool Send(IMessage packet, ushort messageId, int rpcId = -1)
        {
            if (packet == null)
            {
                return false;
            ...
            try
            {
                ...
                m_network.Send(buffer.GetBuffer(), KcpChannel.Reliable);

                return true;
            }
            catch (Exception e)
            {
                Log.Error($"c2s: send data failed, messageId:{messageId}, packet:{packet.GetType().Name}, ex: {e.Message}");
                return false;
            }
        }

        public async Task<IMessage> SendAsync(IMessage packet, ushort messageId)
        {
            if (packet == null)
            {
                return null;
        ...
        private bool Deserialize(ArraySegment<byte> data, out ushort messageId, out int rpcId, out byte[] packetBuffer)
        {
            do
            {
